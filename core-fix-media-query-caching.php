...
function wpcom_vip_media_clear_caches( $id ) {
	...
	if ( strpos( $upload_post->post_mime_type, "video" ) === 0 ) { // Check if it starts with video
		delete_transient( 'wpcom_media_has_video' );//@todo: only delete if cache is false?
	} else if ( strpos( $upload_post->post_mime_type, "audio" ) === 0 ) { // Check if it starts with audio
		delete_transient( 'wpcom_media_has_audio' );//@todo: only delete if cache is false?
	}
}
...
function wpcom_vip_bust_media_months_cache( $post_id ) {
	...
	$months = $wpdb->get_results( $wpdb->prepare( "
			...
			ORDER BY post_date DESC
			LIMIT 1
		", 'attachment' ) ); //This query will run faster than the core one since it orders and returns the first one. @todo: See if we can add the post_status to better use the type_status_date index.
